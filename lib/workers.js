...
workers.log = (originalCheckData, checkOutcome, state, alertUserToStatusChange, timeOfCheck) => {
  ...
  _logs.append(logFileName, logString, (err) => {
    if (!err) {
      console.log(`Logging to the file ${logFileName} succeeded.`);
    } else {
      console.log(`Logging to the file ${logFileName} failed`);
    }
  });
};
        ...
        _logs.compress(logId, newFileId, err => {
          if (!err) {
            ...
            _logs.truncate(logId, err => {
              if (!err) {
                console.log('Sucess truncating the log');
              } else {
                console.log('Error truncating the log');
              };
            });
          } else {
