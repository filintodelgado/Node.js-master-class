      ...
      checks.forEach(check => {
        _data.read('checks', check, (err, originalCheckData) => {
          if (!err && originalCheckData) {
            ...
            debug(`Error reading check ${check} data`);
          };
        });
      });
    } else {
...
workers.alertUserToStatusChange = newCheckData => {
  ...
  helpers.sendTwilioSms(newCheckData.phone, msg, err => {
    if (!err) {
      ...
      debug('Error: Could not send sms alert to user who add a state change in their check');
    };
  });
};

workers.log = (originalCheckData, checkOutcome, state, alertUserToStatusChange, timeOfCheck) => {
  // Form the log data
  var logData = {
    'check'   : originalCheckData,
    'outcome' : checkOutcome,
    'state'   : state,
    'alert'   : alertUserToStatusChange,
    'time'    : timeOfCheck 
  };
  ...
  _logs.append(logFileName, logString, (err) => {
    if (!err) {
      debug(`Logging to the file ${logFileName} succeeded.`);
    } else {
      debug(`Logging to the file ${logFileName} failed`);
    }
  });
};
        ...
        _logs.compress(logId, newFileId, err => {
          if (!err) {
            ...
            _logs.truncate(logId, err => {
              if (!err) {
                debug('Sucess truncating the log');
              } else {
