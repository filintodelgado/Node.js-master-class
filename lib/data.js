/*
 * Library from story and editing data
 */

// Dependencies
const fs = require('fs');
const path = require('path');
const helpers = require('./helpers');

// Container for the module (to be exported)
var lib = {};

// Function to add extension to file
function addExtension(file) {
  file += '.json'
  return file
}

// Base directory of the data folder
lib.baseDir = path.join(__dirname, '/../.data/')

// Write data to a file
lib.create = (dir, file, data, callback)=> {
  // Open the file for writing
  file = addExtension(file);

  fs.open(lib.baseDir+dir+'/'+file, 'wx', (err, fileDescriptor) => {
    if(!err && fileDescriptor) {
      // Convert data to a string
      var stringData = JSON.stringify(data);

      // Write to the file
      fs.writeFile(fileDescriptor, stringData, () => {
        if (!err) {
          fs.close(fileDescriptor, () => {
            if (!err) {
              callback(false);
            } else {
              callback('Error closing new file');
            }
          });
        } else {
          callback('Erro writing to the new file');
        }
      });
    } else {
      callback('Could\'nt create file, it may already exist')
    }
  });
};

// Read the data of a file
lib.read = (dir, file, callback) => {
  file = addExtension(file);

  fs.readFile(lib.baseDir+dir+'/'+file, 'utf-8', (err, data) => {
    if (!err && data) {
      var parseData = helpers.parseJsonToObject(data);
      callback(false, parseData);
    } else {
      callback(err, data);
    }
  });
};

// Update the data of a file
lib.update = (dir, file, data, callback) => {
  file = addExtension(file);

  fs.open(lib.baseDir+dir+'/'+file, 'r+', (err, fileDescriptor) => {
    if (!err && fileDescriptor) {
      // Convert data to a string
      var stringData = JSON.stringify(data);

      // Truncate the file
      fs.ftruncate(fileDescriptor, (err) => {
        if (!err) {
          // Write to the file and close it
          fs.writeFile(fileDescriptor, stringData, (err) => {
            if(!err) {
              fs.close(fileDescriptor, (err) => {
                if (!err) {
                  callback(false);
                } else {
                  callback('Error closing the file')
                }
              });
            } else {
              callback('Error writing to existing file.');
            }
          });
        } else {
          callback('Erro truncating the file')
        }
      });
    } else {
      callback('Could not open the file for updating, it may not exist yet');
    }
  })
};

// Delete a file
lib.delete = (dir, file, callback) => {
  //unlink the file
  file = addExtension(file);

  fs.unlink(lib.baseDir+dir+'/'+file, (err) => {
    if (!err) {
      callback(false)
    } else {
      callback('Error deleting file');
    }
  });
};

// List all the items in a directory
lib.list = (dir, callback) => {
  fs.readdir(lib.baseDir+dir+'/', (err, data) => {
    if (!err && data && data.length > 0) {
      var trimmedFileNames = [];
      
      data.forEach((fileName) => {
        trimmedFileNames.push(fileName.replace('.json', ''));
      });

      callback(false, trimmedFileNames);
    } else {
      callback(err, data);
    }
  });
};

// Export the module
module.exports = lib;